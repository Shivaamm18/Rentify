        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 md:gap-8">
            <div className="max-w-xl">
              <h1 className="text-2xl md:text-4xl font-black text-slate-900 mb-2 md:mb-4 tracking-tight">
            ...
            <div className="flex items-center gap-2 md:gap-3">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 md:px-6 py-3 md:py-3.5 rounded-xl md:rounded-2xl font-bold transition-all shadow-lg text-sm md:text-base ${
                  showFilters 
              ...
              <>
                ...
                >
                  <div className="p-5 border-b border-slate-100 flex items-center justify-between sticky top-0 bg-white">
                    ...
                    <button onClick={() => setShowFilters(false)} className="p-2 hover:bg-slate-100 rounded-xl">
                  ...
                  <div className="p-5 space-y-8">
                    <section>
                      ...
                      <div className="relative group">
                        ...
                        <input
                          type="text"
                          className="w-full pl-11 pr-4 py-3 bg-slate-50 border-none rounded-xl focus:ring-2 focus:ring-primary-500 transition-all text-sm font-bold text-slate-900"
                    ...
                    <section>
                      <h3 className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Property Type</h3>
                    ...
                    <section>
                      <h3 className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Configuration</h3>
                      <div className="grid grid-cols-5 gap-2">
                        {[1, 2, 3, 4, 5].map(n => (
                          <button
                            key={n}
                            onClick={() => handleFilterChange('bhk', filters.bhk === n.toString() ? '' : n.toString())}
                            className={`py-2.5 rounded-xl transition-all text-sm font-black ${
                    ...
                    <section>
                      <h3 className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Budget (Monthly)</h3>
                      <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-2">
                          <label className="text-[10px] font-black text-slate-500 uppercase">Min</label>
                          <input
                            type="number"
                            className="w-full px-3 py-2.5 bg-slate-50 border-none rounded-xl focus:ring-2 focus:ring-primary-500 text-sm font-bold"
                            placeholder="₹0"
                    ...
                    <div className="pt-4 space-y-3">
                      <button
                        onClick={() => {
                          setFilters({ city: '', state: '', minRent: '', maxRent: '', propertyType: '', bhk: '', page: 1 });
                        }}
                        className="w-full py-3 text-sm font-black text-red-500 hover:bg-red-50 rounded-xl transition-all"
                      >
                        Reset All Filters
                      </button>
                      <button
          ...
          <main className="flex-1">
            {loading ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-10">
                {[1, 2, 3, 4].map(i => (
                  <div key={i} className="bg-white rounded-2xl md:rounded-[2rem] h-[350px] md:h-[500px] border border-slate-100 p-4 md:p-7 animate-pulse">
                    <div className="aspect-[4/3] bg-slate-100 rounded-xl md:rounded-3xl mb-4 md:mb-6"></div>
                    <div className="h-5 md:h-6 bg-slate-100 rounded-full w-2/3 mb-3 md:mb-4"></div>
                    <div className="h-4 bg-slate-100 rounded-full w-1/2 mb-6 md:mb-8"></div>
                    <div className="grid grid-cols-2 gap-3 md:gap-4">
                      <div className="h-8 md:h-10 bg-slate-100 rounded-xl md:rounded-2xl"></div>
                      <div className="h-8 md:h-10 bg-slate-100 rounded-xl md:rounded-2xl"></div>
                    </div>
                  </div>
                ))}
              ...
              <motion.div 
                ...
                className="bg-white rounded-2xl md:rounded-[3rem] p-10 md:p-20 text-center border border-slate-100 shadow-premium"
              >
                <div className="bg-primary-50 w-20 h-20 md:w-32 md:h-32 rounded-full flex items-center justify-center mx-auto mb-6 md:mb-10 shadow-inner">
                  <HiSearch className="w-10 h-10 md:w-16 md:h-16 text-primary-200" />
                </div>
                <h3 className="text-xl md:text-3xl font-black text-slate-900 mb-3 md:mb-4">No Matches Found</h3>
                <p className="text-slate-500 font-medium mb-8 md:mb-12 max-w-sm mx-auto leading-relaxed text-sm md:text-lg">
            ...
            {!loading && pagination.pages > 1 && (
              <div className="mt-10 md:mt-20 flex items-center justify-center space-x-2 md:space-x-3">
                <button
                  onClick={() => handlePageChange(pagination.page - 1)}
                  disabled={pagination.page === 1}
                  className="p-3 md:p-4 rounded-xl md:rounded-2xl bg-white border border-slate-200 text-slate-400 disabled:opacity-30 hover:bg-slate-50 transition-all shadow-sm"
                >
                ...
                {Array.from({ length: pagination.pages }, (_, i) => i + 1).map(n => (
                  <button
                    ...
                    className={`w-10 h-10 md:w-14 md:h-14 rounded-xl md:rounded-2xl font-black text-sm md:text-base transition-all ${
                      pagination.page === n
                        ? 'bg-primary-600 text-white shadow-xl shadow-primary-500/40 translate-y-[-2px] md:translate-y-[-4px]'
